...
#### 1.4. Avg spent per month tile renders with a value

**File:** `tests/spending/dashboard-avg-spent-tile.spec.ts`

**Steps:**
  1. Navigate to /dashboard and wait for the page heading
    - expect: Dashboard renders
  2. Locate the 'Avg spent · per month' tile
    - expect: The 'Avg spent · per month' heading is visible
  3. Check the displayed monetary value inside the tile
    - expect: A dollar amount is displayed (not a skeleton loader or '--'); it reflects the average monthly spending across the user's history

#### 1.5. Summary totals update after logging a new discretionary expense
...
#### 1.6. Summary totals update after deleting a discretionary expense
...
#### 3.6. Active trip notice is shown and pre-selects the trip

**File:** `tests/spending/log-expense-active-trip-notice.spec.ts`

**Steps:**
  1. Navigate to /dashboard and click 'Log expense'
    - expect: The 'New expense' panel opens
  2. Look for an info alert banner near the top of the form
    - expect: Note: 'Test Trip' was seeded with past dates so it may not qualify as an 'active trip' (depends on whether its end date is in the future). If no banner appears, skip the active-trip assertions in this test.
  3. Check the 'Linked Trip' select field value
    - expect: If an active trip exists, the Linked Trip field is pre-populated with that trip's name and an info alert reads 'Your current trip, "[name]", has already been applied to this transaction.'
...
#### 9.4. Recurring spend needs-update banner is shown on dashboard when no monthly log exists

**File:** `tests/spending/recurring-needs-update-banner.spec.ts`

**Steps:**
  1. Navigate to /dashboard without logging any recurring transaction for 'Internet'
    - expect: An info alert banner is visible on the dashboard reading '1 recurring expense(s) require updates this month' (or similar text)
  2. Navigate to /recurring_spending
    - expect: The same info banner also appears at the top of the recurring spending page
  3. Log a monthly transaction for 'Internet' (click the Internet card, then add the amount for the current month)
    - expect: Transaction is saved
  4. Navigate back to /dashboard
    - expect: The info banner no longer appears (no more expenses require updates)

#### 9.5. Recurring spend page sidebar shows updated estimated and actual monthly totals
