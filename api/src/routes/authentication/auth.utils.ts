...
export function isEmailAvailable(email: string) {
  return new Promise<void>((resolve) => {
    db.query(`SELECT * FROM user_information.account_info WHERE user_email=${db.escape(email)};`, (error, result) => {
      if (error) {
...
export function isUsernameAvailable(username: string) {
  return new Promise<void>((resolve) => {
    db.query(`SELECT * FROM user_information.account_info WHERE username=${db.escape(username)};`, (error, result) => {
      if (error) {
